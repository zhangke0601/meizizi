...
class Makeoutcome extends Model
{
//    protected $table = '';
    ...
    public function balance_paginate($is_expoter = false)
    {
        ...
        if ($producer_id) {
            $where[] = ['pfdatas.producer_id', '=', $producer_id];
        }
        if ($cartoon_id) {
...
//            ->leftJoin('makeoutcomes', function ($join) {
//                $join->on('makeoutcomes.month', '=', 'pfdatas.month')->on('makeoutcomes.cartoon_id', '=', 'pfdatas.cartoon_id');
//            })
            ->leftJoin('copyrightoutcomes', function ($join) {
                $join->on('copyrightoutcomes.month', '=', 'pfdatas.month')->on('copyrightoutcomes.cartoon_id', '=', 'pfdatas.cartoon_id')->where('copyrightoutcomes.type', '=', 2);
            })
            ->select('pfdatas.*','cartoons.cartoon_name', 'copyrights.copyright_rate','copyrights.company_name','pfdatas.producer_rate','producers.producer_name','copyrightoutcomes.status')
...
//                    'company_name' => $pfdata->company_name,
        ...
        foreach ($final_data as $key => &$value) {
            ...
            $gf = $value['payment'];
            $value['bqfcsj'] =  round(($kfpjs - $all_bqcb - ($bqgs_rate/100)*($kfpjs - $all_bqcb) + $gf*($bqgs_rate/100)) * ($zzfbl/100) / (1-($zzfbl/100)*($bqgs_rate/100)),2);
        }
