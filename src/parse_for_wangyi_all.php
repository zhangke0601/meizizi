...
foreach($lines as $line)
{
    ...
    //foreach($data['data']['books'] as $value)
    {
        ...
        if(!empty($value['labels']))
        {
          ...
          //  if($value['labels'][0]['text'] ===  '独家')
          //      $exclusive = 1;
        }

        $cart->update($ctid, array('ctprogress'=>($value['integrity']==0)?PROGRESS_NORMAL:PROGRESS_OVER));

        $cartdatainfo = array('ctid'=>$ctid, 'ctdsource'=>SOURCE_WANGYI,
            'ctdzancount'=>0, 'ctdbrowsercount'=>$value['hit'],
            'ctdcommentcount'=>$value['commentCount'],
            'ctdredu'=>$value['totalFansValue'],
            ...
            'ctdkeyname'=>str_replace(' ',',', $value['keyWords']),
