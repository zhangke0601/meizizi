...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {
  ...
  this.panel = dom("calendarPanel"); //IE6 BUG
  this.form  = null;
...
Calendar.prototype.draw = function() {
  ...
  var mvAry = [];
  mvAry[mvAry.length]  = '  <form name="calendarForm" id="calendarForm" style="margin: 0px;">';
  ...
  mvAry[mvAry.length]  = '        <th colspan="3"><input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:100%;height:20px;font-size:12px;"/></th>';
  mvAry[mvAry.length]  = '      </tr>';
  mvAry[mvAry.length]  = '    </table>';
  mvAry[mvAry.length]  = '  </form>';
  this.panel.innerHTML = mvAry.join("");
  this.form = dom("calendarForm");
  ...
  this.form.calendarYear.onchange = function () {ieontag=true;calendar.update(this);}
  this.form.calendarMonth.onchange = function () {ieontag=true;calendar.update(this);}
...
Calendar.prototype.bindData = function () {
  ...
  var tds = dom("calendarTable").getElementsByTagName("td");
...
//显示日历
Calendar.prototype.show = function (dateControl,event,callback,popControl) {
  //alert('input.click');
  if(callback!=null){
...
document.onclick = function (evt){
	var event = window.event ? window.event.srcElement : evt.target;
	if(dom('calendarPanel').style.visibility != "hidden" && ieontag==false)//IE+FF
	{
			calendar.hide();
	}
}
