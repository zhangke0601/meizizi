    ...
    */
    class RGrid
    {
        ...
        */
		...
		public function SetCmpCallback($callback)
        {
            $this->colcmp = $callback;
        }

    public function SetVisibleFoot($flag)
    {
      $this->viewfoot = $flag;
    }
        ...
        */
        function Display()
        {
            ...
            */
            if (is_array($this->connection)) {
                if (RGrid::$orderby['column'])
					uasort($this->connection, array('RGrid', '_sortArray'));
...
<table border="0" cellspacing="<?php echo $this->cellspacing ?>" cellpadding="<?php echo $this->cellpadding ?>" class="datagrid" width=95%>
    <thead>
                        ...
                        <th class="col_<?php echo $k ?>" title="<?php echo ($printable = !empty($this->colnames[$h]) ? $this->colnames[$h] : $h) ?>">
                            ...
                            <?php if($this->allowSorting): ?>
                                ...
                                <?php if($h == RGrid::$orderby['column']): ?>
                                    <?php echo (!empty(RGrid::$orderby['direction']) && trim(RGrid::$orderby['direction']) == 1 ? '^' : '')?>
                                    <!--
    ...
    <tbody>
        <?php while($row = (is_array($this->connection) ? current($this->connection) : mysql_fetch_array($this->resultset, MYSQL_ASSOC))):?>
            ...
            <tr <?php $isline = isset($isline)?!$isline:true;if(!$isline) echo 'bgcolor="eeeeaa"';?> onmouseover="MouseOver(this)" onmouseout="MouseOut(this)">
    ...
    <?php }?>
        ...
        */
        public function getQueryString($startnum = null)
        {
            if ($startnum === null) {
            ...
            foreach ($_GET as $k => $v) {
                if(ini_get('magic_quotes_gpc') == "1")
                    $qs .= urlencode($k) . '=' . urlencode(trim(stripslashes($v)))  . '&';
                else
        ...
        private function _sortArray($a, $b)
        {
			...
			if ($this->colcmp)
			{
				if (RGrid::$orderby['direction'])
					return call_user_func($this->colcmp, RGrid::$orderby['column'], $a, $b);
				else
					return -1 * call_user_func($this->colcmp, RGrid::$orderby['column'], $a, $b);
			}
